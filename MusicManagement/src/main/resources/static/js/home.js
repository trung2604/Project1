...
async function fetchRecentlyPlayed() {
    ...
    try {
        const response = await fetch(`/api/recently-played/${userId}`);

        // Check if the response is ok (status code 200-299)
        if (!response.ok) {
            throw new Error(`Failed to fetch recently played songs: ${response.statusText}`);
        }
...
async function updatePlaylistName(playlistId) {
    ...
    try {
        const response = await fetch(`/api/playlists/${playlistId}?newName=${encodeURIComponent(newName)}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' }
        });
...
function playSongFromSource(songId, source, playlistId = null) {
    let url = `/api/songs/play?songId=${songId}&source=${source}`;
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <select id="playlistSelect">
                ${currentPlaylists.map(p => `<option value="${p.playlistId}">${p.playlistName}</option>`).join('')}
            </select>
                ...
                listItem.innerHTML = `
                    <div class="song-container">
                        <div class="image-wrapper">
                            <img src="${song.imgPath || '/images/default.jpg'}" alt="${song.title}" class="song-image">
                        </div>
                        <div class="song-details">
                            <div class="song-info">
                                ...
                                <span class="song-duration">Duration: ${formatDuration(song.duration)}</span>
                            </div>
...
async function removeSongFromPlaylist(playlistId, songId) {
    try {
        const response = await fetch(`/api/playlists/${playlistId}/delete/songs/${songId}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        });
        if (!response.ok) {
...
async function toggleFavorite(songId) {
    ...
    try {
        ...
        const url = isFavorite ? '/api/favourites/remove' : '/api/favourites/add';
            ...
            listItem.innerHTML = `
                <div class="song-container">
                    ...
                    <div class="song-details">
                        ...
                        <div class="song-actions">
                            <button onclick="playSongFromSource(${song.id}, 'favorites')" class="play-btn">Play</button>
                            <button onclick="showAddToPlaylistModal(${song.id})" class="add-to-playlist-btn">Add to Playlist</button>
                            <button onclick="removeFromFavorites(${song.id})" class="remove-btn">Remove</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        listItem.innerHTML = `
            <div class="playlist-container">
                <div class="playlist-info">
                    ...
                    <span class="playlist-date">Created: ${new Date(playlist.createAt).toLocaleDateString()}</span>
                </div>
                <div class="playlist-actions">
                    <button onclick="playPlaylist(${playlist.playlistId})" class="play-btn">View Songs</button>
                    <button onclick="updatePlaylistName(${playlist.playlistId})" class="edit-btn">Edit</button>
                    <button onclick="deletePlaylist(${playlist.playlistId})" class="delete-btn">Delete</button>
                </div>
            </div>
        `;
...
function renderRecentlyPlayed(songs) {
    ...
    songs.forEach(song => {
        ...
        listItem.innerHTML = `
            <div class="song-container">
                <div class="image-wrapper">
                    <img src="${imgSrc}" alt="${songData.title}" class="song-image">
                </div>
                <div class="song-details">
                    <div class="song-info">
                        <span class="song-title" title="${songData.title}">${songData.title}</span>
